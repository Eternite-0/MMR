...
# Please replace the entire compute_pro function in utils/common.py with this one.
...
def compute_pro(anomaly_map: ndarray, gt_mask: ndarray, label: ndarray, num_th: int = 200):
    ...
    # assert gt_mask.ndim == 3, "masks.ndim must be 3 (num_test_data, h, w)" # Buggy assertion
    # assert anomaly_map.shape == gt_mask.shape, "amaps.shape and masks.shape must be same" # Buggy assertion
    ...
    for anomaly_mask, mask in zip(current_amap, current_mask):
        # --- Start: Code Modification to fix FutureWarning ---
        # Instead of creating an empty DataFrame and appending, we collect rows in a list.
        rows_list = []
        # --- End: Code Modification ---

        binary_amaps = np.zeros_like(anomaly_mask, dtype=bool)
        ...
        for th in np.arange(min_th, max_th, delta):
            ...
            # --- Start: Code Modification to fix FutureWarning ---
            # Append the new row data as a dictionary to our list.
            rows_list.append({"pro": mean(pros) if pros else 0, "fpr": fpr, "threshold": th})
            # --- End: Code Modification ---

        # --- Start: Code Modification to fix FutureWarning ---
        # After the loop, create the DataFrame from the list of rows all at once.
        ...
        pro_auc = auc(df["fpr"], df["pro"]) if not df.empty else 0
