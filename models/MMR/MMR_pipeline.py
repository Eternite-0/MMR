...
class MMR_pipeline_:
    ...
    def __init__(self,
        ...
        for extract_layer in cfg.TRAIN.MMR.layers_to_extract_from:
            # CORRECTED: The variable is now spelled correctly as "extract_layer"
    ...
    def fit(self, individual_dataloader):
        ...
        # --- Start: Correct placement for model saving code ---
        # CORRECTED: The saving block is now OUTSIDE and AFTER the epoch loop.
        if self.cfg.TRAIN.save_model:
            checkpoint_dir = os.path.join(self.cfg.OUTPUT_DIR, "checkpoints")
            os.makedirs(checkpoint_dir, exist_ok=True)

            save_path = os.path.join(checkpoint_dir, "mmr_model_final.pth")

            torch.save(self.mmr_model.state_dict(), save_path)
            LOGGER.info(f"Training complete. Model saved to: {save_path}")
        # --- End: Correct placement for model saving code ---
